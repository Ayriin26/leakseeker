...
class SecretScanner:
    ...
    def is_false_positive(self, matched_text: str, pattern_name: str) -> bool:
        ...
        # Check if it's obviously a test/example value (exact match only)
        if matched_text.lower() in false_positives:
            return True

        # Bug fix: the original check `'example' in matched_text.lower()` was too
        # aggressive — it silently dropped real secrets whose value happened to
        # contain the substring "example" (e.g. EXAMPLE in an AWS key template).
        # Only filter on explicit placeholder suffixes/prefixes instead.
        lower = matched_text.lower()
        placeholder_markers = ('_placeholder', 'your_secret', 'your_key', 'changeme', 'xxxx')
